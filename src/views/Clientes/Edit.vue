...
const form = ref({ id: '', nombre: '', correo: '', telefono: '', edad: '' });
...
const getCliente = () => {
    axios.get('/api/clientes/' + id.value).then(response => {
        form.value.nombre = response.data.data.nombre;
        form.value.correo = response.data.data.correo;
        form.value.telefono = response.data.data.telefono;
        form.value.edad = response.data.data.edad;
    });
};

const save = () => {
    sendRequest('PUT', form.value, ('/api/clientes/' + id.value), '/clientes');
};
</script>
                ...
                <div class="card-body">
                    <form @submit.prevent="save">
                        ...
                        <div class="input-group mb-3">
                            ...
                            <input type="number" v-model="form.edad" placeholder="Edad" class="form-control border-0" style="background: #f7d794;" required>
                        </div>
